...
router.post("/generate-recommendations", async (req, res) => {
  try {
    ...
    const userId = req.user._id;
    const nutritionContext = await buildNutritionContext(userId);
...
When suggesting meals, ALWAYS use this exact structure for each meal:
{
  ...
  "tags": ["string"] (array of 1-3 diet/restriction-friendly tags like gluten-free, high-protein, etc.)
}
`;

    const systemPrompt = `You are NutriGenie, a personalized AI nutrition assistant.
You are generating a fresh list of 3-4 personalized meal recommendations for a user.
You are provided with a NUTRITION CONTEXT generated from the user's profile, recent meals, daily stats, age, weight, height, BMI, dietary preferences, and allergies.

${mealSchemaReference}

STRICT OUTPUT RULES:
- Respond ONLY with a single valid JSON array containing exactly 3 to 4 meal objects.
- Do NOT include backticks, markdown code blocks (like \`\`\`json), or explanations. Just return the raw JSON array.
- The JSON array MUST contain objects matching the MEAL RECOMMENDATION SCHEMA exactly.

BEHAVIOR RULES:
- Respect the user's diet type, dietary restrictions, and allergies completely.
- Avoid repeating meals that the user has already eaten recently (listed in NUTRITION CONTEXT).
- Make sure recommendations are delicious, nutritious, and perfectly match their calculated BMI and macro goals.
...
${nutritionContext}`;
    ...
    try {
      ...
      mealsList = match ? JSON.parse(match[0]) : JSON.parse(cleanText);
    } catch (err) {
      console.error("Failed to parse dynamic recommendations:", err, result.text);
    ...
    const savedMeals = [];
    for (const rec of mealsList) {
      if (!rec.mealName || !rec.mealType) continue;

      const validMealTypes = ["breakfast", "lunch", "dinner", "snack"];
      const type = validMealTypes.includes(rec.mealType.toLowerCase())
        ? rec.mealType.toLowerCase()
        : "snack";
      ...
      const recommendedMeal = await Meal.create({
        ...
        mealType: type,
        status: "recommended",
        calories: Math.max(0, Number(rec.calories) || 0),
        protein_g: Math.max(0, Number(rec.protein_g) || 0),
        carbs_g: Math.max(0, Number(rec.carbs_g) || 0),
        fat_g: Math.max(0, Number(rec.fat_g) || 0),
        fiber_g: Math.max(0, Number(rec.fiber_g) || 0),
        sugar_g: Math.max(0, Number(rec.sugar_g) || 0),
        sodium_mg: Math.max(0, Number(rec.sodium_mg) || 0),
        description: rec.description || "Freshly recommended healthy meal",
        tags: Array.isArray(rec.tags) ? rec.tags : [],
        notes: "Generated by NutriGenie AI Recommendation Engine",
      });
      savedMeals.push(recommendedMeal);
    }
